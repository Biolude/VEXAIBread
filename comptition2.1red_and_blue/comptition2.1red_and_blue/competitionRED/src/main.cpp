...
void RedLG_Shoot(int shooter_time)//红方LongGoal筛球发射，括号内为发射时间，单位毫秒
{
  ...
  while(shooter_time_now <= shooter_time/20)
  {
    Intake.spin(fwd,100,pct);
    DownRoller.spin(fwd,100,pct);
    if(!(Shooter_Optical.hue() > 160 and Shooter_Optical.hue() < 240))
    {
      UpRoller.spin(fwd,60,pct);
    }
    if(!(Shooter_Optical.hue() > 140 and Shooter_Optical.hue() < 230))
    {
      Shooter.spin(fwd,100,pct);
    }
    else if(Shooter_Optical.hue() > 140 and Shooter_Optical.hue() < 230)
    {
      Shooter.spin(fwd,-100,pct);
    }
...
void runTrackingAndGPSCycle() { // 函数名可调整，更清晰
  ...
  while (global_cycle_count < MAX_GLOBAL_CYCLES || MAX_GLOBAL_CYCLES == 0) {
    ...
    if (!is_tracking_stuck) {
      ...
      RedLeftShoot(); // 执行GPS定位与发射（已移除位置重置）
    } else {
...
void auto_Interaction(void) {
  ...
  RedLG_Shoot(5000); // 执行LongGoal发射逻辑（可选，可根据需求调整）
}
